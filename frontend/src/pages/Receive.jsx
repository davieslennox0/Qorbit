    ...
    <div className="max-w-lg space-y-6">
      ...
      <form onSubmit={handleRegister} className="space-y-4 rounded-lg border border-border bg-surface p-5">
        ...
        <button
          ...
          className="w-full rounded-md bg-cyan text-black font-medium text-sm py-2.5 hover:brightness-110 disabled:opacity-50"
        >
      ...
      {registerError && (
        <div className="rounded-lg border border-red-900 bg-red-950/40 px-4 py-3 text-sm text-red-300">{registerError}</div>
      )}
      {registerResult && (
        <div className="rounded-lg border border-emerald-900 bg-emerald-950/30 px-4 py-3 text-sm text-emerald-300 font-mono">
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          {watchAddress && incoming.length === 0 && (
            <div className="text-sm text-slate-600">No incoming payments yet for this address.</div>
          )}
          {incoming.map((p) => (
